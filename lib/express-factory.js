...
Express = instanceFactory.createClass('Express', {
  ...
  methods: {
    init: function (opt) {
      ...
      that.app = express();
      if (typeof that.config.set === 'object') {
        Object.keys(that.config.set).forEach(function (key) {
          log.trace('setting express app property %s', key);
          that.app.set(key, that.config.set[key]);
        });
      }
      if (typeof that.config.engine === 'object') {
        Object.keys(that.config.engine).forEach(function (key) {
          log.trace('setting express app engine %s', key);
          that.app.engine(key, that.config.engine[key]);
        });
      }
      if (that.config.log) {
